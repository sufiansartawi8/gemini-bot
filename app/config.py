...
class Settings(BaseSettings):
    ...
    auto_start_bot: bool = Field(default=False, alias='AUTO_START_BOT')
    enable_private_api: bool = Field(default=False, alias='ENABLE_PRIVATE_API')
    ...
    check_interval_seconds: int = Field(default=900, alias='CHECK_INTERVAL_SECONDS')
    candle_interval: str = Field(default='15m', alias='CANDLE_INTERVAL')
    lookback_candles: int = Field(default=180, alias='LOOKBACK_CANDLES')
    ...
    def private_api_ready(self) -> bool:
        return bool(self.enable_private_api and self.gemini_api_key.strip() and self.gemini_api_secret.strip())
