...
class TradingBot:
    ...
    def run_cycle(self) -> dict[str, Any]:
        balances, account_mode = self._load_balances()
        ...
        for symbol in settings.parsed_symbols:
            ...
            if decision.action == 'buy' and open_positions >= settings.max_open_positions:
                decision = Decision('hold', symbol, 'max open positions reached', last_price, meta=decision.meta)

            executed = None
            if decision.action == 'buy':
                amount = self._size_buy(last_price, balances)
                if amount > 0:
                    ...
                    decision = Decision('hold', symbol, 'insufficient USD for new position', last_price, meta=decision.meta)
        ...
        self.last_run = {
            ...
            'decisions': decisions,
            'ts': int(time.time()),
            'mode': 'live' if settings.private_api_ready and not settings.dry_run else 'dry_run',
    ...
    def _load_balances(self) -> tuple[list[dict[str, Any]], str]:
        if not settings.private_api_ready:
            return [], 'public_only'
        try:
            return self.client.get_balances(), 'private'
        except GeminiAPIError as exc:
            logger.warning('private Gemini API unavailable, falling back to dry-run sizing: %s', exc)
            return [], 'private_error_fallback'
    ...
    def _execute(self, decision: Decision) -> dict[str, Any] | None:
        ...
        if not settings.private_api_ready:
            logger.warning('skipping live order because private API is not enabled')
            return None
    ...
    def _estimate_entry_prices(self) -> dict[str, float]:
        ...
        if not settings.private_api_ready or settings.dry_run:
            return {symbol: 0.0 for symbol in settings.parsed_symbols}

        for symbol in settings.parsed_symbols:
            try:
                trades = self.client.get_trades(symbol, limit_trades=50)
            except Exception:
                trades = []
            ...
            if not buys:
                prices[symbol] = 0.0
                continue
            total_qty = sum(float(t.get('amount', 0) or 0) for t in buys if float(t.get('amount', 0) or 0) > 0)
    ...
    def _size_buy(self, last_price: float, balances: list[dict[str, Any]]) -> float:
