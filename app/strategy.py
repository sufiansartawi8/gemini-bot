    ...
    def evaluate(self, symbol: str, candles: list[Candle], position: Position | None) -> Decision:
        ...
        if position and position.amount > 0:
            ...
            if pnl_pct <= -settings.stop_loss_pct:
                return Decision('sell', symbol, f'stop loss hit ({pnl_pct:.2%})', price, meta=meta)
            if pnl_pct >= settings.take_profit_pct:
                return Decision('sell', symbol, f'take profit hit ({pnl_pct:.2%})', price, meta=meta)
            if bearish_cross or rsi_now < settings.rsi_sell_min:
                return Decision('sell', symbol, f'bearish exit: cross={bearish_cross}, rsi={rsi_now:.1f}', price, meta=meta)
            return Decision('hold', symbol, f'position held: rsi={rsi_now:.1f}', price, meta=meta)

        if bullish_cross and rsi_now < settings.rsi_buy_max:
            return Decision('buy', symbol, f'bullish entry: cross={bullish_cross}, rsi={rsi_now:.1f}', price, meta=meta)

        return Decision('hold', symbol, f'no setup: cross={bullish_cross}, rsi={rsi_now:.1f}', price, meta=meta)
